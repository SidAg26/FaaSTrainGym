...
class Environment(gym.Env):
    ...
    def _take_action(self, action):
        try:
            ...
            current_pods = self.ctx.get_k8s_scale_api().read_namespaced_deployment(name=self.ctx.get_k8s_deployment_name(),
                                            namespace=self.ctx.get_k8s_deployment_namespace()).status.ready_replicas
        ...
        if action < 0 :
            if (scale_value >= self.MIN_PODS):
                ...
                try:
                    _ = self.ctx.get_k8s_scale_api().patch_namespaced_deployment_scale(name=self.ctx.get_k8s_deployment_name(), 
                                                    namespace=self.ctx.get_k8s_deployment_namespace(),
    ...
    def _get_obs(self):
        ...
        try:
            data = self.ctx.get_prometheus_api().custom_query(query=query1)
        ...
        try:
            ...
            data = self.ctx.get_prometheus_api().custom_query(query=query3)
        ...
        try:
            ...
            data = self.ctx.get_prometheus_api().custom_query(query=query4)
            total = 0
            for d in data:
        ...
        try:
            ...
            query2 = "increase(gateway_function_invocation_total{code='200', function_name='" + self.ctx.get_k8s_deployment_name() + "." + self.ctx.get_k8s_deployment_namespace() + "'}[30s])"
            data = self.ctx.get_prometheus_api().custom_query(query=query2)
        ...
        # get the avg usage metrics
            resource_list = self.ctx.get_k8s_resource_api().list_namespaced_custom_object("metrics.k8s.io", "v1beta1", "openfaas-fn", "pods")
            my_pods  = [pod['containers'][0]['usage'] for pod in resource_list['items'] if pod['metadata']['labels']['faas_function'] == self.ctx.get_k8s_deployment_name()]
            cpu = 0
            mem = 0
            for pods in my_pods:
                c = pods['cpu']
